...
public class TestGlicko2 {
    ...
    public void test() {
        ...
        Assert.assertEquals(0, player1.getGlicko2Rating(), 0.00001);
        Assert.assertEquals(1.1513, player1.getGlicko2RatingDeviation(), 0.00001);
        ...
        Assert.assertEquals(1464.06, player1.getRating(), 0.01);
        Assert.assertEquals(151.52, player1.getRatingDeviation(), 0.01);
        Assert.assertEquals(0.05999, player1.getVolatility(), 0.01);

        // test that opponent 4 has had appropriate calculations applied
        Assert.assertEquals(ratingSystem.getDefaultRating(), player5.getRating(), 0);  // rating should be unaffected
        Assert.assertTrue(ratingSystem.getDefaultRatingDeviation() < player5.getRatingDeviation());  // rating deviation should have grown
        Assert.assertEquals(ratingSystem.getDefaultVolatility(), player5.getVolatility(), 0);  // volatility should be unaffected
    }
